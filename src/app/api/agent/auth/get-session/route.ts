...
export async function GET() {
    try {
        ...
        if (!sessionToken) {
            ...
            return new Response(JSON.stringify({ error: "No session token", valid: false }), {
                status: 401,
        ...
        if (!isPresent) {
            ...
            if (!notRevoked) {
                console.warn("[get-session] FAIL → Session revoked at:", session.revokedAt);
            }
            if (!notExpired) {
                console.warn("[get-session] FAIL → Session expired at:", session.expiresAt);
            }
        }

        if (!valid) {
            return new Response(JSON.stringify({ error: "Invalid or expired session", valid }), {
                status: 401,
                headers: { "Content-Type": "application/json" },
            });
        }

        console.log("[get-session] SUCCESS → Valid session for agent:", session.agentId);
        ...
        return new Response(JSON.stringify({ error: "Server error", valid: false }), {
            status: 500,
            headers: { "Content-Type": "application/json" },
        });
    }
