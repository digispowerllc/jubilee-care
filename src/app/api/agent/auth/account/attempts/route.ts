...
export async function POST(req: Request) {
  try {
    ...
    if (typeof attempts !== "number" || attempts < 0) {
      console.warn(
        `[${new Date().toISOString()}] Invalid attempts value from agent ${session.id}:`,
        attempts
      );
    ...
    console.info(
      `[${new Date().toISOString()}] Recorded deletion attempt for agent ${session.id}, attempts: ${attempts}`
    );
    ...
    if (
      ...
      new Date(profile.deletionLockedUntil) > new Date()
    ) {
      console.info(
        `[${new Date().toISOString()}] Agent ${session.id} attempted deletion but account is locked until ${profile.deletionLockedUntil}`
      );
      return NextResponse.json({
    ...
    if (attempts >= SECURITY_CONFIG.MAX_ATTEMPTS) {
      ...
      console.warn(
        `[${new Date().toISOString()}] Agent ${session.id} account locked until ${lockedUntil} due to too many failed deletion attempts`
      );
    }
    ...
    console.error(
      `[${new Date().toISOString()}] Failed to process deletion attempt:`,
      error
    );
    return NextResponse.json(
  ...
  try {
    ...
    const recentAttempts = await prisma.failedDeletionAttempt.count({
      where: {
        ...
        createdAt: {
          gte: new Date(
            Date.now() - SECURITY_CONFIG.WINDOW_HOURS * 3600 * 1000
          ),
        },
      },
    });
    ...
    console.info(
      `[${new Date().toISOString()}] Agent ${session.id} checked deletion attempt status: recentAttempts=${recentAttempts}, locked=${isLocked}`
    );
    ...
    return NextResponse.json({
      ...
      attemptsRemaining: isLocked
        ? 0
        : Math.max(SECURITY_CONFIG.MAX_ATTEMPTS - recentAttempts, 0),
    });
  } catch (error) {
    console.error(
      `[${new Date().toISOString()}] Failed to get attempt status:`,
      error
    );
    return NextResponse.json(
