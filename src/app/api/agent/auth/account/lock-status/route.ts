...
export async function GET() {
    ...
    try {
        ...
        if (!session) {
            console.warn(`[${now.toISOString()}] Unauthorized lock status check attempt`);
        ...
        console.info(`[${now.toISOString()}] Fetching lock status for agent ${agentId}`);
        ...
        const [activeLock, profile, recentAttempts] = await prisma.$transaction([
            ...
            prisma.failedDeletionAttempt.count({
                where: {
                    agentId,
                    createdAt: {
                        gte: new Date(now.getTime() - SECURITY_CONFIG.WINDOW_HOURS * 3600 * 1000)
                    }
                },
            }),
        ]);
        ...
        const deletionLockedUntil = activeLock?.expiresAt ?? profile?.deletionLockedUntil;
        const lockedUntil = systemLockedUntil || deletionLockedUntil;

        const attemptsRemaining = Math.max(SECURITY_CONFIG.MAX_ATTEMPTS - recentAttempts, 0);
        ...
        console.info(`[${now.toISOString()}] Agent ${agentId} lock status:`, {
            ...
            attemptsRemaining,
            lockType: activeLock?.action ?? (profile?.isLocked ? 'ACCOUNT_LOCK' : null)
        });
        ...
        return NextResponse.json({
            ...
            attemptsRemaining,
            lockType: activeLock?.action ?? (profile?.isLocked ? 'ACCOUNT_LOCK' : null),
            nextAttemptAllowed: attemptsRemaining <= 0 ?
                new Date(now.getTime() + SECURITY_CONFIG.WINDOW_HOURS * 3600 * 1000).toISOString() :
                null
        });
    } catch (error) {
