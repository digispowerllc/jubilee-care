...
import { verifyResetToken, generateResetToken } from "@/lib/utils/auth-utils";
  ...
  try {
    ...
    if (!token || !sid) {
      return NextResponse.json(
        { success: false, message: "Missing token or agent ID" },
        { status: 400 }
      );
    }
    ...
    if (!isValid || !agentId) {
      return NextResponse.json(
        {
          success: false,
          message: error || "Invalid token. Request a new reset link.",
        },
        { status: 400 }
      );
    }
    ...
    writeToLogger(
      "info",
      `Verified account: ${agentId}, generated reset token`
    );
    ...
    return NextResponse.json(
      { success: false, message: "Server error", code: "SERVER_ERROR" },
      { status: 500 }
    );
  }
}
